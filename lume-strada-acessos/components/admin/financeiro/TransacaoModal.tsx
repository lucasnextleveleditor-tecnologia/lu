...
export function TransacaoModal({
  ...
  // Conta bancária ("Pagar com: Conta", nunca Cartão) NUNCA é perguntada
  // aqui — nem quando "Já paga" está marcado — pedido explícito do dono da
  // conta: a conta só é escolhida na hora REAL de "Dar Baixa" (ver
  // `DarBaixaModal`/`marcarPago`), sem exceção, porque as coisas podem mudar
  // até lá e pode existir mais de uma conta pessoal/empresarial (fácil de
  // errar escolhendo cedo demais). A ÚNICA exceção é editar uma transação
  // que JÁ tinha uma conta resolvida antes (deu baixa alguma vez, ou é de
  // antes dessa mudança existir) — aí continua editável normalmente, sem
  // forçar reabrir/dar baixa de novo só pra corrigir um detalhe.
  const contaJaResolvida = Boolean(editando?.conta_id);
  const precisaContaAgora = fonte === "conta" && contaJaResolvida;
  // "Já paga" só faz sentido aqui quando a conta/cartão já é conhecido no
  // ato de lançar (cartão, transferência, ou edição de algo já resolvido) —
  // pra uma despesa/receita nova em "Conta", marcar "já paga" sem saber de
  // qual conta saiu não faz sentido, então nem oferecemos a opção: ela nasce
  // pendente e vira paga só quando você der baixa escolhendo a conta.
  const mostrarJaPaga = tipo === "transferencia" || fonte === "cartao" || contaJaResolvida;
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const payload = {
      ...
      dataVencimento,
      // Sem checkbox visível (`!mostrarJaPaga`) a transação nasce sempre
      // pendente, mesmo que `jaPaga` ainda guarde o `true` inicial por baixo
      // dos panos — ver comentário em `mostrarJaPaga` acima.
      jaPaga: mostrarJaPaga ? jaPaga : false,
