...
export default async function NovoContratoPage({ searchParams }: PageProps) {
  ...
  const [{ clientes, tiposContrato, orcamentosParaVincular, empresa, logoContratoUrl }, nomeEmpresa] = await Promise.all([buscarDadosConstrutorContrato(), getNomeApp()]);
  ...
  return (
    <div className="space-y-6">
      ...
      <ContratoBuilder nomeEmpresa={nomeEmpresa} empresa={empresa} logoContratoUrl={logoContratoUrl} clientes={clientes} tiposContrato={tiposContrato} orcamentosParaVincular={orcamentosParaVincular} orcamentoIdInicial={orcamentoId} />
    </div>
  );
}
