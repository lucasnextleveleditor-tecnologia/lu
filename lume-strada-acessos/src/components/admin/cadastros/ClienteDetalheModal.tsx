      ...
      >
        <div className="mb-5 flex items-start justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-ink-muted">{cliente.documento || "Sem CNPJ/CPF cadastrado"}</p>
          </div>
        ...
        <div className="mb-6 grid grid-cols-1 gap-x-6 gap-y-3 rounded-xl border border-base-800 bg-base-950/40 p-4 sm:grid-cols-2">
          <div>
            <p className="text-[11px] uppercase tracking-wide text-ink-muted">E-mail de Contato</p>
            <p className="text-sm text-ink-primary">{cliente.email || "—"}</p>
          </div>
          <div>
            <p className="text-[11px] uppercase tracking-wide text-ink-muted">Telefone / WhatsApp</p>
            <p className="text-sm text-ink-primary">{cliente.telefone || "—"}</p>
          </div>
          <div>
            <p className="text-[11px] uppercase tracking-wide text-ink-muted">Responsável</p>
            <p className="text-sm text-ink-primary">{cliente.nome_responsavel || "—"}</p>
          </div>
          <div className="sm:col-span-2">
            <p className="text-[11px] uppercase tracking-wide text-ink-muted">Endereço</p>
            <p className="text-sm text-ink-primary">{cliente.endereco || "—"}</p>
          </div>
        </div>

        <div className="mb-6">
          <h4 className="mb-3 text-sm font-semibold text-ink-primary">Acesso ao Dashboard do Cliente</h4>
          {profile ? (
            ...
            <div className="flex flex-col items-start gap-2.5 rounded-xl border border-dashed border-base-700 p-4">
              <p className="text-xs text-ink-muted">Este cliente ainda não tem acesso ao dashboard (Tráfego, Aprovações, Boletos).</p>
              {souAdmin && (
        ...
        <div>
          <h4 className="mb-3 text-sm font-semibold text-ink-primary">Atividades & Tarefas</h4>
