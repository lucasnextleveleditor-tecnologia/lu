...
export function FinanceiroReport({ data, carregando, erro }: FinanceiroReportProps) {
  ...
  if (data.qtdTransacoes === 0) {
    return <RelatorioEmptyState titulo={dict.relatorios.financeiroVazioTitulo} descricao={dict.relatorios.financeiroVazioDescricao} />;
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-ink-primary">{dict.relatorios.financeiroTitulo}</h2>
          <p className="text-xs text-ink-muted">{dict.relatorios.financeiroSubtitulo}</p>
        </div>
        <ExportMenuButton
          ...
          colunasCSV={[
            ...
            { chave: "receitas", rotulo: dict.relatorios.financeiroCsvReceitas },
            { chave: "despesas", rotulo: dict.relatorios.financeiroCsvDespesas },
            { chave: "saldo", rotulo: dict.relatorios.financeiroCsvSaldo },
          ]}
        />
      </div>
      ...
      <div id="relatorio-financeiro-export" className="space-y-5 rounded-2xl bg-base-950 p-1">
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          />
          <StatTile icon={IconWallet} label={dict.relatorios.financeiroStatLancamentos} value={data.qtdTransacoes} hint={dict.relatorios.financeiroStatLancamentosHint} />
        </div>

        <Card>
          <h3 className="mb-4 text-sm font-semibold text-ink-primary">{dict.relatorios.financeiroFluxoCaixaTitulo}</h3>
          ...
          <div className="mt-3 flex items-center gap-4 text-xs text-ink-secondary">
            <span className="flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: CHART_CORES.receita }} /> {dict.relatorios.financeiroLegendaReceitas}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: CHART_CORES.despesa }} /> {dict.relatorios.financeiroLegendaDespesas}
            </span>
          </div>
        </Card>
        ...
        {data.porCategoria.length > 0 && (
          <Card>
            <h3 className="mb-4 text-sm font-semibold text-ink-primary">{dict.relatorios.financeiroDespesasCategoriaTitulo}</h3>
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
