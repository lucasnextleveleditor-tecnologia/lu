...
export function ProducaoReport({ data, carregando, erro }: ProducaoReportProps) {
  ...
  if (erro) return <RelatorioEmptyState titulo={dict.relatorios.producaoErroTitulo} descricao={erro} />;
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-ink-primary">{dict.relatorios.producaoTitulo}</h2>
          <p className="text-xs text-ink-muted">{dict.relatorios.producaoSubtitulo}</p>
        </div>
        <ExportMenuButton
          ...
          dadosCSV={csvLinhas}
          colunasCSV={[
            { chave: "funcionario", rotulo: dict.relatorios.producaoCsvFuncionario },
            { chave: "tarefasConcluidas", rotulo: dict.relatorios.producaoCsvTarefasConcluidas },
          ]}
        />
      </div>
      ...
      <div id="relatorio-producao-export" className="space-y-5 rounded-2xl bg-base-950 p-1">
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatTile icon={IconClipboardList} label={dict.relatorios.producaoStatCriadas} value={data.tarefasCriadasNoPeriodo} />
          <StatTile icon={IconCheckCircle} label={dict.relatorios.producaoStatConcluidas} value={data.tarefasConcluidasNoPeriodo} tone="good" />
          <StatTile
            icon={IconAlertTriangle}
            label={dict.relatorios.producaoStatGargalos}
            value={data.tarefasAtrasadas}
            tone={data.tarefasAtrasadas > 0 ? "critical" : "neutral"}
            hint={dict.relatorios.producaoStatGargalosHint}
          />
          <StatTile
            icon={IconTarget}
            label={dict.relatorios.producaoStatTempoConclusao}
            value={data.tempoMedioConclusaoDias !== null ? `${Math.round(data.tempoMedioConclusaoDias)} ${dict.relatorios.diasSufixo}` : "—"}
            hint={dict.relatorios.producaoStatTempoConclusaoHint}
          />
        </div>

        {semDadosDoPeriodo ? (
          <RelatorioEmptyState titulo={dict.relatorios.producaoVazioTitulo} descricao={dict.relatorios.producaoVazioDescricao} />
        ) : (
          <Card>
            <h3 className="mb-4 text-sm font-semibold text-ink-primary">{dict.relatorios.producaoProdutividadeTitulo}</h3>
            {data.produtividade.length === 0 ? (
              <p className="py-6 text-center text-sm text-ink-muted">{dict.relatorios.producaoSemConcluidas}</p>
            ) : (
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.produtividade} layout="vertical" margin={{ top: 4, right: 24, left: 8, bottom: 0 }}>
                    ...
                    <Tooltip {...CHART_TOOLTIP_STYLE} formatter={(valor: number) => [valor, dict.relatorios.producaoTooltipConcluidas]} />
