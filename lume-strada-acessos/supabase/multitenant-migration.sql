...
-- uma empresa "Empresa Principal" criada por este script — nada se perde.
--
...
-- ----------------------------------------------------------------------------
-- 2.1 Backfill — cria a empresa "Empresa Principal" (a agência dona do
...
declare
  v_empresa_id uuid;
begin
  select id into v_empresa_id from public.companies where nome = 'Empresa Principal' limit 1;

  if v_empresa_id is null then
    insert into public.companies (nome, status)
    values ('Empresa Principal', 'ativo')
...
--   b) UPDATE ... SET company_id = <empresa principal> WHERE company_id IS
...
-- cria a nova; a linha que já existia (da empresa principal) continua valendo como
-- a sessão da empresa principal.
