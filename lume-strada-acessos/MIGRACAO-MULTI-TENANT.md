...
- `generateLink` faz só a metade que precisa da Service Role (criar o usuário) e devolve o link pronto (`action_link`) — sem nenhum envio de e-mail, então nunca esbarra em rate limit. Cada modal de "Gerar acesso" agora mostra esse link com um botão **Copiar link** e um atalho **Abrir no WhatsApp** (componente `LinkAcessoGerado`, em `src/components/ui/LinkAcessoGerado.tsx`) — copie e mande pra quem for (WhatsApp, e-mail manual, etc.).
- O link gerado é exatamente o mesmo formato que o e-mail automático mandava antes (`inviteUserByEmail` por baixo dos panos também só embrulhava um `generateLink`) — `src/app/auth/callback/route.ts` não precisou mudar.
